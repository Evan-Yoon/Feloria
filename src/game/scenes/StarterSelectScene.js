...
export class StarterSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .text(width / 2, 100, "함께 모험을 떠날 고양이를 선택하세요:", {
        font: "24px Arial",
        fill: "#ecf0f1",
      })
      .setOrigin(0.5);
  ...
  createStarterOption(data) {
    ...
    const typeLabel = this.add
      .text(0, 160, data.type, { font: "16px Arial", fill: "#ffffff" })
      .setOrigin(0.5);

    container.add([sprite, label, typeLabel]);

    const hitArea = this.add
      .rectangle(0, 0, 150, 200, 0xffffff, 0)
      .setInteractive({ useHandCursor: true });
  ...
  confirmStarter() {
    ...
    const starterInstance = battleSystem.createInstance(
      this.selectedId.toUpperCase(),
      5,
    );
    ...
    // Mark as caught in Codex
    codexSystem.markCaught(this.registry, this.selectedId.toUpperCase());
