...
export class EvolutionScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.oldCreatureId) {
        this.oldSprite = this.add.sprite(cx, cy, this.oldCreatureId.toLowerCase()).setScale(6);
    } else {
        this.oldSprite = this.add.sprite(cx, cy, "player").setScale(6);
    }
    
    // New Sprite (Hidden initially)
    if (this.newCreatureId) {
        this.newSprite = this.add.sprite(cx, cy, this.newCreatureId.toLowerCase()).setScale(6).setAlpha(0);
        this.newSprite.setTintFill(0xffffff); // Start as white silhouette
    }

    // Light Glow Effect
    this.glow = this.add.circle(cx, cy, 10, 0xffffff, 0).setBlendMode(Phaser.BlendModes.ADD);
