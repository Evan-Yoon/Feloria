import fs from "fs";
import { CREATURES } from "./src/game/data/creatures.js";
import { SKILLS as existingSkills } from "./src/game/data/skills.js";
...
// 1. Assign 4th skill to legendaries
...
for (const leg of legendaries) {
  if (CREATURES[leg] && CREATURES[leg].skills.length === 3) {
    CREATURES[leg].skills.push(legExtraSkills[leg]);
    modifiedCreatures = true;
  }
}

// Write back creatures if modified
if (modifiedCreatures) {
  const newCreaturesText =
    "export const CREATURES = " +
    JSON.stringify(CREATURES, null, 2).replace(/"([^"]+)":/g, "$1:") +
    ";\n";
...
for (const [id, c] of Object.entries(CREATURES)) {
  ...
  if (legendaries.includes(id)) {
    ...
    grade = 1; // Assuming all terminal forms are grade 1
  }
...
// Generate remaining to exactly 200
let fillerId = 1;
while (Object.keys(allSkills).length < 200) {
  const sid = "filler_skill_" + fillerId;
  const grade = 2; // Make fillers grade 2 conceptually
  allSkills[sid] = {
    id: sid,
    name: "비전기 " + fillerId,
    grade: grade,
...
// Format safely to match export format
